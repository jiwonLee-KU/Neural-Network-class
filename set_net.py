...
def set_net(args, net, train_loader, val_loader, test_loader):
    print("[Make solver]")
    # Set criterion
    ...
    test_log_dict = test_set_metrics(args)

    # Set solver
    module = importlib.import_module(f"Solver.{args.net}_solver")
    solver = module.Solver(args, net, train_loader, val_loader, test_loader, criterion, optimizer, scheduler, log_dict, test_log_dict)
    print("")
    return solver
...
def test_set_metrics(args):
    if args.train_cont_path:
        print("continue log_dict")
        test_log_dict = read_json(os.path.join(os.path.dirname(os.path.dirname(args.train_cont_path)), "log_dict.json"))
    else:
        test_log_dict = {f"{phase}_{metric}": [] for phase in ["test"] for metric in args.metrics}
